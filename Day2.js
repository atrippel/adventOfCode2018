...
const encontrarCaracteresDuplicados = (str) => {
    ...
    unicos.forEach(letra =>{
        ocurrencias.push(caracteres.filter(caracter => caracter === letra).length);
    });

    if(ocurrencias.some(elem => elem === 2) && ocurrencias.some(elem => elem === 3)){
        return 'both';
    }else if(ocurrencias.some(elem => elem === 2)){
        return 'two';
    }else if (ocurrencias.some(elem => elem === 3)){
        return 'three';
    }else {
        return 'none';
    }
...
const buscarFactores = (cadenas) => {
    let two = 0;
    let three = 0;
    
    cadenas.forEach(cadena => {
        switch (encontrarCaracteresDuplicados(cadena)) {
        case 'both':
            two += 1;
            three += 1;
            break;
